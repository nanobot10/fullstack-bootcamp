...
const AsyncExample = () => {
    ...
    function showStorage() {
        saveSessionStorage('name', 'Dennis')
            .then((response) => {
                let value = response;
                alert(`Saved name: ${value}`);
            })
            .catch((error) => alert(`Something went wrong: ${error}`))
            .finally(() => alert('Name saved and retrieved succesfuly'));
    }
    ...
    const urlNotFound = async() => {
        try {
            ...
            alert(`Something went wrong witn your URL ${error}`)
        }
    }
    ...
    const multiplePromise = async() => {
        ...
        ).catch((error) => alert(`Something went wrong witn your URL ${error}`));
    }
